...
**"투명함"을 이미지에서 재는 두 기준 (탭 `판정:` 콤보):**

| 기준 | 원리 | 언제 쓰나 |
|---|---|---|
| **배경 유사도** (`bg`, 기본) | 맑은 플라스틱 **너머로는 케이스가 놓인 바닥이 비쳐 보인다**. 그래서 힌지 쪽 띠의 색 분포가 **케이스 바깥(바닥) 분포와 닮는다**. 바닥 표본은 마스크를 팽창(dilate)해서 만든 **케이스 둘레 링**에서 뜬다 — 바로 옆 바닥이라 조명 조건이 같다 | 기본. 물리적 근거가 뚜렷 |
| **이질도** (`odd`) | "힌지 변만 나머지 셋과 다르다"는 사실만 쓴다. 각 띠가 **다른 세 띠와 얼마나 다른지**를 재서 가장 튀는 변을 힌지로 | 바닥 색이 케이스 본체와 비슷해 `bg` 대비가 안 나올 때. 단 **인쇄 로고가 있는 변이 대신 튈 수 있음** |

**비교는 평균색이 아니라 히스토그램으로** 한다. 평균만 보면 "밝은 점 + 어두운 점"과 "중간 회색"이 같은 값이 되어 구분되지 않기 때문이다. 채널별 정규화 히스토그램(24 bin)을 이어붙인 벡터를 만들고 **피어슨 상관**으로 닮은 정도를 잰다. **컬러 이미지가 있으면 3채널을 모두 쓴다** — 투명부는 바닥의 밝기뿐 아니라 **색**까지 닮으므로 판별력이 크게 오른다 (탭이 `rgb=self.current_rgb` 를 넘긴다). gray 만 있어도 동작한다.
...
- **띠 두께%** (`opening_hinge_band_spin`, 5–45, 기본 25): 변 안쪽에서 투명함을 재는 띠 두께(케이스 크기 대비). **얇으면** 표본이 적어 노이즈에 흔들리고, **두꺼우면** 가운데 제품 영역이 네 띠에 공통으로 섞여 변 사이 차이가 흐려진다. **실제 투명 힌지부의 폭에 맞추는 게 원칙.** 0.45 를 넘겨 넣어도 함수가 클램프한다(띠가 서로 겹치면 판별 자체가 불가능).
