...
3. **띠마다 "투명함 점수"** — 아래 두 기준 중 선택. 둘 다 **점수가 높을수록 힌지**가 되도록 부호를 맞췄다.
...
**"투명함"을 이미지에서 재는 세 기준 (탭 `판정:` 콤보):**

| 기준 | 원리 | 언제 쓰나 |
|---|---|---|
| **본체 색 대비** (`chroma`, **기본·권장**) | 띠의 색이 **케이스 본체 색에서 얼마나 벗어났나**를 Lab 색상면(a\*b\*) **거리**로 잰다. 힌지 쪽은 투명하거나(바닥이 비침) 금속이라 **중성(회색)** 에 가깝고, 나머지 세 변은 본체 색(예: 분홍) 그대로다. 본체 표본은 띠를 뺀 **안쪽 영역의 중앙값** | 기본. **배경을 안 쓰므로 그림자에 덜 속고**, '거리'라서 조금 다름/많이 다름이 구분된다 |
| **배경 유사도** (`bg`) | 맑은 플라스틱 **너머로 바닥이 비쳐 보인다** → 힌지 쪽 띠의 색 분포가 **케이스 바깥(바닥) 분포와 닮는다**. 바닥 표본은 마스크를 팽창해 만든 **둘레 링**(바로 옆 바닥이라 조명 조건 동일) | 힌지가 실제로 바닥을 비추고 색차가 뚜렷할 때 |
| **이질도** (`odd`) | "힌지 변만 나머지 셋과 다르다"는 사실만 쓴다 | 위 둘이 안 될 때. **인쇄 로고가 있는 변이 대신 튈 수 있음** |

`chroma` 는 색을 **Lab 로 바꿔 밝기(L)를 버리고 색상면(a\*b\*)만** 본다. 투명부는 배경 색을 보이되 플라스틱이 빛을 먹어 **어두워지므로**, 밝기를 그대로 비교하면 본체와 구별이 안 되지만 색상면에서는 뚜렷이 갈리기 때문이다. 컬러가 없으면 밝기 편차로 대체하지만 판별력이 크게 떨어지므로 **컬러를 넘기는 것을 전제로 한다** (탭이 `rgb=self.current_rgb` 를 넘긴다).

`bg`/`odd` 는 채널별 정규화 히스토그램(24 bin)을 이어붙인 벡터의 **피어슨 상관**으로 닮은 정도를 잰다.

> ⚠️ **왜 기본값을 바꿨나 (실측 근거):** 피어슨 상관은 두 색 분포가 **겹치지 않으면 "얼마나 먼지"와 무관하게 비슷한 값으로 포화**된다. 실제 캡처(분홍 정사각 케이스, 배경 RGB 130/129/128 · 힌지 103/94/96 · 본체 94/68/66)에서 `bg`/`odd` 의 네 변 점수가 `0.15~0.18` 안에 몰려 사실상 동점이 됐고 — **정지 장면인데도 실행마다 답이 바뀌고 정반대로도 나왔다**. 같은 사진에서 `chroma` 는 `[3.43, 0.89, 1.37, 1.91]` 로 1등이 2등의 1.8배(띠를 힌지 폭에 맞추면 2.3배)라 띠 두께 10~30% 전 구간에서 흔들림 없이 같은 답을 냈다. 이 사진은 `tests/data/hinge_case.png` 로 저장해 회귀 테스트(`tests/test_opening_hinge_real.py`)에 고정했다.
>
> **`chroma` 의 약점:** 배경 대신 본체를 기준으로 삼으므로, **마스크에 그림자가 섞이면** 그 변이 중성 회색이라 '힌지'로 오인된다(실측에서 확신하며 오답). 마스크 경계는 짧은 변의 4%만큼 침식해 걷어내지만 — 힌지 띠 자체가 가장자리에 붙어 있어 더 깎으면 신호까지 사라지므로 — **마스크 품질이 이 방식의 전제**다. SAM3 마스크가 케이스를 정확히 감싸는지 디버그 뷰에서 확인할 것.

**신뢰도(confidence)** 는 다른 방식과 의미가 다르다. 4지선다에서는 "얼마나 확실히 **하나만** 튀는가"가 곧 신뢰도이므로, 1등과 2등의 **절대 격차**를 쓴다 (점수가 상관값이라 절대값이 그대로 의미를 가진다):

```
confidence = clip(1등 − 2등, 0, 1)
```
...
**0.15 미만이면** 네 변이 고만고만하다는 뜻이다. 가장 흔한 원인은 **마스크가 투명한 힌지부를 포함하지 못해** 네 띠가 전부 케이스 본체인 경우 — 이러면 무엇을 골라도 사실상 찍는 것이라 결과가 매 실행마다 바뀐다(정지 장면 20회 반복 실험에서 정답률 3/20, 네 방향에 고르게 분산). 상태바에 값의 범위를 `힌지 판별 격차 0.32~0.71` 형태로 표시하고, 0.15 미만이면 "신뢰도 낮음 N개" 로 경고한다(방식마다 점수 스케일이 달라 임계값도 다르다 — 무게중심/격자는 0.02).
...
- **판정** (`opening_hinge_metric_combo`): 위 표의 `bg`/`odd`.
...
**보는 법:** 힌지 변 하나만 막대가 길고 나머지 셋이 비슷하면 잘 동작하는 것이다(1등과 2등의 격차 = `conf`). 엉뚱한 변이 빨강이면 ① 왼쪽 패널에서 빨간 띠가 **실제 투명부를 덮고 있는지** 보고 `띠 두께%` 를 맞추고, ② 그래도 안 되면 `판정` 을 `이질도` 로 바꿔 본다.
