...
- 기본 트래커는 **BoT-SORT(ReID 제외)** — ByteTrack 의 2단계 연관(고신뢰 → 저신뢰)에 GIoU 유사도와 (cx,cy,w,h) 칼만을 더한 것. **CMC(카메라 모션 보정)는 기본 꺼짐**: 이 시스템은 카메라가 고정(eye-to-hand)이라 보정할 움직임이 없어 잡음만 는다. eye-in-hand 구성으로 바꾸면 `use_cmc=True` 로 켠다.
