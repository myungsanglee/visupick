...
### 3.2.1 SAM3 실시간 검출 (데모 영상 촬영용)

2행의 **"▶ SAM3 실시간 검출"** 버튼은 `캡처 → SAM3 → 2D 표시`를 반복해 인식 장면을 **영상처럼** 보여준다. 빈 픽킹 동작에는 쓰이지 않는 **데모 촬영 전용** 기능이다. 다시 누르면 멈춘다.

설계상 지킨 두 가지:

**① 실행 중에는 로봇이 움직이지 않는다.** 실시간 루프는 프레임마다 화면을 갈아엎으므로, 그 사이에 픽/이동이 돌면 **화면에 보이는 것과 로봇이 가는 좌표가 어긋난다**. 그래서:
- `RobotControlMixin._check_motion_allowed()` 를 **모든 모션 진입점**(`_execute_move` / `_move_to_taught`(Home·놓기) / `_run_cycle`(픽·시퀀스) / `_auto_start`(연속 픽))에서 호출한다. 탭이 `_motion_blocked_reason()` 을 오버라이드해 실시간 중이면 사유를 돌려준다.
- 버튼 비활성화(`_set_motion_controls_enabled`)는 **보조 수단**이다 — 단축키·타이머 콜백처럼 버튼을 거치지 않는 경로가 있어 하드 가드가 본체다. 해제할 때는 잠글 때 기억해 둔 상태로 되돌린다(무조건 켜면 로봇 미연결 등으로 원래 꺼져 있어야 할 버튼까지 켜진다).
- **비상정지·비상정지 해제·큐 비우기는 잠그지 않는다.** 잠긴 상태에서도 멈출 수 있어야 하므로 `MOTION_BUTTONS` 목록에서 의도적으로 제외했다.

**② Bin Box(ROI) 필터는 정식 경로와 똑같이 적용된다.** 작업 영역 밖 물체가 데모 영상에 잡히면 안 되기 때문. 필터는 `_filter_by_roi()` 하나로 공유하므로 두 경로의 규칙이 갈라지지 않는다 — bbox 가 **전부** ROI 안에 들어와야 통과(부분 걸침 제외). 다만 실시간은 속도 때문에 포인트클라우드를 뽑지 않으므로 **깊이 밴드(2차 게이트)는 자동 생략**되고 2D 사각형 게이트만 적용된다. 걸러낸 개수는 상태바에 `(ROI 밖 N개 제외)` 로 표시하고, Bin Box 폴리곤은 화면에 그대로 남아 영상에 작업 영역이 보인다.

**③ 정식 캡처 상태를 건드리지 않는다.** `current_image`/`current_xyz`·`detections`·`pick_objects`·3D 뷰는 그대로 두고 **2D 오버레이만** 갱신한다. 멈추면 마지막 정식 캡처 화면으로 되돌린다 — 데모를 찍고 나서 평소 작업을 그대로 이어갈 수 있게. 3D 포즈 계산과 포인트클라우드 추출도 건너뛰어 프레임 시간을 아낀다.

추론은 **UI 스레드**에서 돈다. SAM3 는 CUDA 컨텍스트가 스레드에 묶여 있어 워커로 옮기는 것이 위험하기 때문([cad_matching.md](cad_matching.md) 9.5 절의 같은 이유). 대신 `QTimer` 로 **한 틱에 한 프레임만** 처리하고 이벤트 루프로 돌아가므로, 프레임 사이에 정지·비상정지가 반응한다. 상태바에 `검출 수 / 추론 ms / fps` 를 표시한다.

> 검출기(`Sam3Detector`)는 단발 검출과 **같은 인스턴스를 재사용**한다(`_sam3_detect`). 모델 로드가 수십 초라 매 프레임 새로 만들면 실시간이 불가능하다.

**추적(tracking).** 실시간 검출 결과는 [tracking.py](../tracking.py) 의 트래커를 거쳐 **같은 물체에 같은 `ID`** 가 붙는다. 화면·영상의 라벨이 `#3 case 0.91` 형태가 되고, **색도 `track_id` 기준**이라 한 물체가 촬영 내내 같은 색을 유지한다(검출 순서로 색을 고르면 프레임마다 깜빡인다).

- [tracking.py](../tracking.py) 에는 트래커가 **3종** 있고 셋 다 칼만 + 헝가리안 할당을 공유한다. **연관 전략만 다르다**:

| 트래커 | 연관 방식 | 언제 |
|---|---|---|
| `SortTracker` | 1단계만 (임계 이상 검출을 한 번에) | 가장 단순 — 비교 기준 |
| `ByteTrackTracker` | 2단계 (고신뢰 → 저신뢰) | 검출이 잠깐 흐려져도 트랙 유지 |
| `BotSortTracker` ✅**기본** | 2단계 + GIoU + CMC 옵션 | 교차 시 ID 뒤바뀜이 가장 적음 |

- 실시간 검출은 `create_tracker("botsort", min_hits=2)` 를 쓴다 — ByteTrack 의 2단계 연관에 GIoU 유사도와 (cx,cy,w,h) 칼만을 더한 것. **CMC(카메라 모션 보정)는 기본 꺼짐**: 이 시스템은 카메라가 고정(eye-to-hand)이라 보정할 움직임이 없어 잡음만 는다. eye-in-hand 구성으로 바꾸면 `use_cmc=True` 로 켠다.
- **ROI 로 거른 뒤에** 추적한다 — 작업 영역 밖 물체에 번호를 주면 ID 가 낭비되고, 영역을 들락거릴 때마다 트랙이 생겼다 사라진다.
- `min_hits=2` 로 둔다. 기본값 3 이면 실시간이 수 fps 라 물체가 나온 뒤 ID 가 붙기까지 1초쯤 걸려 데모가 답답하다. 2 면 한 프레임짜리 깜빡임 오검출은 걸러내면서 거의 바로 표시된다.
- 실시간을 시작할 때마다 트래커를 새로 만들고 `Track.reset_ids()` 로 번호를 1 부터 돌린다 (영상에 큰 번호가 뜨지 않게).

**영상 저장 (mp4).** 실시간 검출을 켜면 옆에 **"⏺ 실시간 영상 저장"** 버튼이 나타난다(그 전에는 녹화할 대상이 없으므로 숨김). 누르면 `원본 저장`·`렌더링 저장`과 **같은 파일 이름 창**(`ask_debug_filename`)이 떠서 이름을 정하고, 그 시점부터 `data/debug_captures/` 에 녹화한다. **"⏹ 실시간 중지"** 를 누르면 그때까지의 영상을 저장하고 실시간도 끝낸다.

기록하는 그림은 **오버레이까지 그려진 화면**(`_make_overlay_image()`)이라 `렌더링 저장`과 같은 모습이다 — bbox·마스크·Bin Box 폴리곤이 모두 담긴다.

> **재생 속도를 실시간과 맞추는 법:** SAM3 추론이 느려 프레임이 드문드문 들어오는데(보통 수 fps) `cv2.VideoWriter` 는 생성 시점에 fps 가 고정된다. 그래서 **출력 15fps 로 고정하되, 경과 시간만큼 같은 그림을 복제해 채운다**(`_rec_write`). 이렇게 하지 않고 한 틱에 한 프레임씩만 쓰면 30초 촬영이 6초짜리 빨리감기 영상이 된다. 한 프레임이 메꿀 수 있는 최대치는 `REC_MAX_DUP` 으로 제한한다(검출이 오래 멈칫할 때 파일이 폭주하지 않게).
>
> 코덱은 `mp4v` — 이 환경에서 `avc1`/`H264` 는 FFmpeg 가 거부한다(확인함).
>
> **VideoWriter 는 첫 프레임에서 만든다.** 컨테이너를 열려면 해상도를 알아야 하는데, 최초 1회 SAM3 모델 로드가 수십 초 걸려 그 사이에 저장 버튼을 누르면 아직 화면이 없다. 그때 "녹화할 화면이 없습니다" 로 거절하면 사용자는 영문을 모르므로, 이름만 받아 **예약**해 두고 첫 프레임에서 파일을 만든다.
...
**여는 축은 단축으로 고정한다 (단, 가로로 긴 케이스에 한해):** 화장품 클램셸은 **힌지와 립(여는 쪽)이 둘 다 긴 변**이다. 따라서 여는 방향은 두 긴 변 사이 = **짧은 extent 축(단축)** 을 향한다. 여는 축 자체는 OBB 의 단축으로 못박고, 이음선 비대칭은 **그 축의 부호(둘 중 어느 긴 변이 립인가)** 만 정한다. (여는 축까지 자동 선택하게 두면, 인쇄·힌지 금속 같은 잡음이 장축 오프셋을 키워 엉뚱하게 짧은 변 쪽으로 화살표가 나가는 문제가 있었다.)

> ⚠️ **정사각형 케이스는 이 전제가 깨진다.** 네 변 길이가 같으면 `minAreaRect` 의 단축은 픽셀 노이즈로 정해져 프레임마다 뒤집히고, 애초에 힌지가 올 수 있는 변이 **4개**라 "단축의 부호"라는 2택 구조로는 **원리적으로 풀 수 없다**. 그래서 네 변을 모두 평가하는 별도 방식이 필요하다 → 아래 **힌지 투명도 방식**.
...
**네 가지 방식(탭 3행 "여는 방향 방식" 콤보에서 선택):**

| 방식 (`method`) | 경우의 수 | 원리 | 부호 의미 |
|---|---|---|---|
| **내부 격자 비대칭** (`"grid"`) ✅가로로 긴 케이스 권장 | 2 (단축 부호) | 투명 케이스 내부 **칸 배열(2×N)이 한쪽으로 치우쳐** 반대쪽에 빈 여백 띠가 생기는 걸 이용. 가로로 긴 케이스 실측에서 가장 강건 | 넓은 여백 쪽(제품마다 다름 → 반전) |
| **힌지 투명도** (`"hinge"`) ✅**정사각형 케이스 전용** | **4 (네 변)** | 힌지가 있는 변만 **본체와 다르게 보인다**(투명해서 바닥이 비치거나 금속). 네 변 안쪽 띠를 모두 평가해 가장 다른 변 = 힌지, **그 반대쪽 = 여는 방향**. 색/밝기 두 단서를 계산해 판별이 되는 쪽을 자동 채택 | 힌지의 맞은편이 립 (반전 불필요) |
| **이음선 에지** (`"seam"`) | 2 (단축 부호) | Sobel 에지 크기 `√(gx²+gy²)` 무게중심. 뚜껑-바닥 이음선(여는 쪽+양옆 U자)의 에지가 여는 쪽에 몰림 | 에지 많은 쪽 = 립 |
| **내부 밝기 비대칭** (`"brightness"`) | 2 (단축 부호) | 밝기 `gray − min(gray)` 무게중심. 투명 내부(팬/거울/힌지)의 밝기가 앞뒤로 비대칭 | 밝은 쪽(제품마다 다름 → 반전) |

`seam`/`brightness` 는 **공통 뼈대**(`opening_analysis.opening_from_weight`)를 쓰고 가중치 맵만 다르다. `grid`(`opening_from_grid`)와 `hinge`(`opening_from_hinge`)는 각각 별도 경로다 — 아래 전용 절 참고. **`hinge` 만 여는 축이 자유롭고**, 나머지 셋은 OBB 단축 고정 + 부호만 정한다.

> **방식 분기는 탭의 `_compute_opening()` 한 곳에 모여 있다** ([bin_picking_tab.py](../bin_picking_tab.py)). 일괄 계산(`_detect_opening`)과 자동 선택 폴백(`_direction_for`)이 같은 경로를 쓰게 하기 위해서다 — 예전에는 폴백이 방식과 무관하게 `opening_from_weight` 를 불러서, 격자 방식을 골라 둬도 **연속 픽의 자동 선택에서는 이음선 계산이 나가는** 버그가 있었다.
...
**2D 뷰 표시 (`image_view.DraggableImageLabel`):** 회전 사각형(OBB) + **중심 → 여는 쪽 화살표** + 화살표 시작점의 **중심 원**(흰 테두리 2px — 어두운 배경이나 같은 색 객체 위에서도 묻히지 않게) + **정보 라벨**.

```
"OBB 검출" 만 눌렀을 때 :  X: 412.0, Y: 233.0
"여는 방향" 까지 눌렀을 때 :  X: 412.0, Y: 233.0, Deg: -20.0, Open: 0.42
```

**`Deg` 는 여는 방향 각도다 (OBB 각도가 아니다).** 이미지 좌표계에서 **중심 → 여는 쪽** 벡터의 `atan2` 로, 범위는 **−180°~180°**:

| angle_deg | 화살표가 가리키는 쪽 |
|---|---|
| `0°` | 오른쪽 (+x) |
| `+90°` | **아래** |
| `±180°` | 왼쪽 |
...
기준축은 이미지의 **가로축(오른쪽)**, 부호는 이미지 y축이 아래를 향하므로 **화면상 시계방향이 +** 다. 여는 방향을 아직 구하지 않았으면(“OBB 검출” 만 누른 경우) `Deg`·`Open` 은 붙지 않고 중심 좌표만 나온다.

> **왜 OBB 각도를 안 쓰나:** `cv2.minAreaRect` 가 주는 각도(`obb["angle"]`)는 `w` 변이 +x 축에서 시계방향으로 돈 각인데 **0~90° 로 접혀 있다**(OpenCV 4.5+ 규약). 그래서 ① 90° 마다 값이 되돌아가 물리적으로 다른 자세가 같은 값을 내고(회전 0°와 90° 둘 다 `90.0`), ② 경계에서 튄다(회전 89.5°→`0.6`, 90.0°→`90.0`). 물체가 어느 쪽을 향하는지 알려주지 못하므로 라벨에서 뺐다. 정사각형 케이스에서는 `w` 가 어느 변이 될지 픽셀 노이즈로 정해져 더더욱 의미가 없다.
>
> 두 각도 모두 **이미지 좌표계** 기준이다. 로봇이 실제로 회전하는 base 좌표계의 A 값은 이 방향을 작업 평면에 투영해 따로 계산한다(`_yaw_from_direction`, §6.7 Grasp 설정의 `회전 소스`).

라벨은 위 항목을 한 줄에 모은 것으로, 글자 길이에 맞춘 사각형을 **그 객체의 색으로 채우고 흰 글씨**로 그린다(문구 조립은 `_compose_label`, 그리기는 `_draw_info_label`) — 객체가 여럿일 때 bbox·마스크와 같은 색이라 어느 라벨이 어느 객체 것인지 바로 매칭된다.

**위치와 그리기 순서** (둘 다 가림 방지):
- 라벨의 **왼쪽 아래 모서리**를 객체 중심(=화살표 시작점)에서 오른쪽·위로 `LABEL_OFFSET_X/Y`(각 8px) 띄운다. 중심에 딱 맞추면 화살표 시작점을 덮어 버린다 — 오른쪽 위에 띄워 두면 화살표가 어느 방향으로 나가든(아래·왼쪽이 흔함) 시작점이 보인다.
- **화살표를 먼저 그리고 라벨을 나중에** 그린다. OBB 루프에서는 `pending_labels` 에 모으기만 하고, 화살표 루프가 끝난 뒤 최상단에 올린다 — 반대 순서면 긴 화살표가 글씨 위를 지나 가린다. 중심 원도 같은 이유로 화살표 **뒤에** 그린다(화살표 꼬리에 가려지지 않게).
- 중심 원은 **OBB 오버레이에서도** 모은다 — "OBB 검출" 버튼처럼 여는 방향 없이 OBB 만 계산했을 때도 중심이 보여야 하기 때문. OBB 와 화살표가 같은 중심을 가리키면 좌표로 중복을 걸러 한 번만 그린다. 선택된 객체는 초록으로 강조된다. `Open:` 은 여는 방향을 계산했을 때만 붙는다(OBB 만 계산한 경우 생략). 이미지 가장자리 객체는 라벨이 잘리지 않도록 캔버스 안으로 밀어 넣는다.

> 예전에는 각도만 중심에 쓰고 신뢰도는 **화살표 촉 옆**에 따로 그렸는데, 화살표가 짧으면 두 글자가 겹쳐 읽기 어려웠다. 지금은 중심 라벨 하나로 합쳤다.
...
#### 힌지 투명도 방식 (`hinge`) — 정사각형 케이스 전용

**왜 필요한가:** 앞의 세 방식은 전부 "여는 축 = OBB 단축"을 못박고 **부호(2택)** 만 정한다. 가로로 긴 클램셸은 힌지와 립이 둘 다 긴 변이라 이게 맞지만, **정사각형 케이스**는 네 변 길이가 같아 단축이 픽셀 노이즈로 정해진다 — 프레임마다 축이 뒤집히고, 힌지가 올 수 있는 변이 **4개**라 2택 구조로는 못 푼다.

**단서:** 실제 제품에서 **힌지가 있는 변만 투명**하다(나머지 세 변은 제품·인쇄·프레임으로 불투명). 그러니 "네 변 중 투명한 변"을 찾으면 그게 힌지고, **맞은편이 여는 쪽**이다.

`opening_analysis.opening_from_hinge` 절차:

1. **OBB 로 warp** — 케이스를 똑바로 세운다. 마스크도 같이 warp 해서, OBB 모서리 중 실제 객체가 아닌 부분(마스크 밖)은 계산에서 뺀다. canonical 좌표의 네 변은 순서대로 **0=위, 1=오른쪽, 2=아래, 3=왼쪽**.
2. **네 변 안쪽 띠(band) 추출** — 각 변에서 안쪽으로 `띠 두께%` 만큼. **모서리는 잘라낸다**: 모서리는 두 띠에 동시에 들어가 인접한 변끼리 점수가 섞이고, 그러면 1등과 2등의 격차(=신뢰도)가 줄어 판별이 흐려진다.
3. **띠마다 "투명함 점수"** — 아래 "본체 색 대비" 참고. **점수가 높을수록 힌지**다.
4. **힌지 = 점수 최대인 변**, 여는 방향 = **맞은편 변**의 바깥 방향. canonical 중심과 그 변의 중점 두 점을 원본으로 **역투영**해 원본 이미지 좌표계의 단위벡터로 되돌린다(이후 3D 변환·grasp 계산과 좌표계를 맞추기 위해).

**"투명함"을 이미지에서 어떻게 재나 — 본체로부터의 편차:**

띠의 픽셀이 **케이스 본체에서 얼마나 벗어났나**를 Lab 거리로 잰다. 본체 표본은 띠를 뺀 **안쪽 영역의 중앙값**(인쇄·요철에 덜 흔들리게 median). 힌지 쪽은 투명하거나(바닥이 비침) 금속이라 본체와 다르고, 나머지 세 변은 본체 그대로다.

**두 단서를 모두 계산해 자동으로 고른다.** 제품마다 힌지가 "무엇으로" 달라지는지가 다르기 때문이다:

| 단서 | 무엇을 보나 | 이 단서로 풀리는 제품 |
|---|---|---|
| **색** (`color`, Lab a\*b\*) | 색상면 거리 | **분홍 케이스** — 본체가 분홍, 힌지는 중성 회색 |
| **밝기** (`lightness`, Lab L) | 밝기 거리 | **검은 케이스** — 본체가 무채색이라 색으로는 신호가 없고, 투명한 힌지가 훨씬 밝다 |

각 단서의 점수는 **본체 내부의 자연스러운 산포(MAD)로 나눠 정규화**한다. 그래야 채널 스케일이 달라도 비교할 수 있고, **원래 많이 흔들리는 채널이 자동으로 깎인다** — 케이스 요철 그늘 때문에 밝기가 들쭉날쭉하면 밝기 쪽 점수가 통째로 눌린다. 그다음 **1·2등 격차가 큰 쪽**을 채택한다 ("실제로 판별이 되는 단서를 쓴다"). 결과 dict 의 `feature` 에 어느 쪽이 쓰였는지 담긴다.

실측 2종 × 띠 두께 6종 = **12가지 설정에서 모두 정답**이고, 매번 기대한 단서가 선택된다:

| | 선택된 단서 | 신뢰도(띠 8~30%) |
|---|---|---|
| 분홍 케이스 | `color` | 1.00 → 0.15 |
| 검은 케이스 | `lightness` | 1.00 (전 구간) |

> **왜 자동 선택이 필요했나 (실제 사례):** 검은 `rom&nd` 케이스는 **아래쪽에 흰색 로고가 인쇄**돼 있다. 색만 보면 그 흰 글씨 때문에 아래 변이 1등이 되어 힌지로 뽑히고, 여는 방향이 **정반대**로 나왔다. 반대로 분홍 케이스에서 밝기만 보면 왼쪽 요철 그늘에 속아 역시 오답이 난다. **한 단서로는 두 제품을 모두 풀 수 없다.** 사용자가 고르게 두는 대신(제품마다 바꿔줘야 함) 알고리즘이 격차를 보고 스스로 정하게 했다.

> **컬러가 없으면** 밝기 단서만 쓴다 — 색으로 갈리는 제품은 판별할 수 없으므로 호출부는 되도록 `rgb` 를 넘긴다 (탭이 `rgb=self.current_rgb` 를 넘긴다).

> **제거된 대안 (실측 근거):** 예전에는 `배경 유사도`(띠가 바닥 색 분포와 닮은 정도)와 `이질도`(띠가 나머지 세 띠와 다른 정도)를 `판정` 콤보로 고를 수 있었다. 둘 다 히스토그램의 **피어슨 상관**으로 쟀는데, 이 지표는 두 분포가 겹치지 않으면 **"얼마나 먼지"와 무관하게 비슷한 값으로 포화**된다. 분홍 케이스 실측에서 네 변 점수가 `0.15~0.18` 안에 몰려 사실상 동점이 됐고 — **정지 장면인데도 실행마다 답이 바뀌고 정반대로도 나왔다.** 8회 반복 비교에서 `배경 유사도`는 실제 제품(0/8)과 검은 케이스(3/8)에서 실패했고, `이질도`는 합성에선 통했지만 실물에선 설정 따라 답이 튀었다. 선택지로 남기면 "그럴듯한데 틀리는" 길을 열어 두는 셈이라 제거했다.

> **약점 — 마스크 품질이 전제다:** 본체를 기준으로 삼으므로, **마스크에 그림자가 섞이면** 그 변이 본체와 달라 '힌지'로 오인된다(실측에서 확신하며 오답). 마스크 경계는 짧은 변의 **4%만큼 침식**해 걷어내지만 — 힌지 띠 자체가 가장자리에 붙어 있어 더 깎으면 신호까지 사라지므로 — 넓은 그림자까지는 못 막는다. SAM3 마스크가 케이스를 정확히 감싸는지 디버그 뷰에서 확인할 것.
>
> ⚠️ **침식 구현 주의:** `cv2.erode` 의 **기본 경계값은 최댓값**이라 이미지 가장자리가 깎이지 않는다. warp 마스크는 OBB 를 꽉 채우므로 기본값으로 부르면 **침식이 통째로 무효**가 된다(픽셀 수가 그대로). 실제로 이 버그 때문에 마스크가 2px 만 삐져나와도 그 변이 배경색을 머금어 점수가 뛰었고, 근거 없는 합성 장면에서 신뢰도가 0.45까지 나왔다. `borderType=cv2.BORDER_CONSTANT, borderValue=0` 을 반드시 넘긴다 (고친 뒤 같은 장면이 `0.005~0.026`).

**신뢰도(confidence)** 는 다른 방식과 의미가 다르다. 4지선다에서는 "얼마나 확실히 **하나만** 튀는가"가 곧 신뢰도이므로, 1등과 2등의 **절대 격차**를 Lab 색차 단위로 환산해 쓴다:

```
confidence = clip((1등 − 2등) / HINGE_CONF_MARGIN, 0, 1)      # 상수 = 1.5
```

점수가 본체 내부 산포(MAD)로 정규화돼 있으므로 "격차 1.5 = 본체가 자연히 흔들리는 폭의 1.5배만큼 그 변이 튄다"는 뜻이다. 실측에서 근거 있는 경우 1.4~15, 근거 없는 경우 0.1 미만이라 그 사이로 잡았다.

> **수정 이력 (중요):** 예전에는 `(1등 − 2등) / (1등 − 4등)` 로 **전체 산포에 정규화**했는데, 이러면 스케일이 사라져 **네 변이 거의 동점일 때도 비율만 크면 높은 값**이 나왔다. 실제로 마스크가 투명한 힌지부를 못 덮어 점수가 `0.115 / 0.092 / 0.092 / 0.091` 로 사실상 동점이던 경우에 `conf = 0.96` 이 나와, **찍은 답을 확신하는 것처럼** 보였다. 절대 격차로 바꾸니 같은 상황이 `0.02` 로 떨어져 낮은 신뢰도가 제대로 드러난다.

**0.15 미만이면** 네 변이 고만고만하다는 뜻이다. 가장 흔한 원인은 **띠 두께가 실제 힌지 폭과 안 맞거나**, 마스크가 투명한 힌지부를 포함하지 못해 네 띠가 전부 케이스 본체인 경우 — 이러면 무엇을 골라도 사실상 찍는 것이라 결과가 매 실행마다 바뀐다(정지 장면 20회 반복 실험에서 정답률 3/20, 네 방향에 고르게 분산). 상태바에 값의 범위를 `힌지 판별 격차 0.32~0.71` 형태로 표시하고, 0.15 미만이면 "신뢰도 낮음 N개" 로 경고한다(방식마다 점수 스케일이 달라 임계값도 다르다 — 무게중심/격자는 0.02).

**결과가 매번 바뀐다면** 이 순서로 확인한다:
1. 디버그 뷰(`VISUPICK_OPENING_DEBUG=1`)의 **막대 네 개가 고만고만한지** — 그렇다면 판별 근거 자체가 없는 것이다.
2. 왼쪽 패널의 warp 에 **투명한 힌지부가 들어와 있는지**. 안 들어와 있으면 SAM3 마스크가 그 부분을 뺀 것이므로 **프롬프트를 바꿔** 투명부까지 포함되게 한다(예: `rectangle` → `cosmetic case`).
3. 마스크는 맞는데 신호가 약하면 `띠 두께%` 를 실제 투명부 폭에 맞춘다.

**조정값 (탭 3행, 힌지 방식 선택 시만 표시):**
- **띠 두께%** (`opening_hinge_band_spin`, 5–45, 기본 15): 변 안쪽에서 투명함을 재는 띠 두께(케이스 크기 대비). **얇으면** 표본이 적어 노이즈에 흔들리고, **두꺼우면** 가운데 제품 영역이 네 띠에 공통으로 섞여 변 사이 차이가 흐려진다. **실제 힌지부의 폭에 맞추는 것이 조정값 중 효과가 가장 크다** — 실측 캡처에서 힌지 바는 케이스 높이의 약 **8%** 였고, 25% 로 두면 신호가 3배 희석돼 신뢰도가 `0.73 → 0.11` 로 떨어졌다(방향 자체는 유지). 0.45 를 넘겨 넣어도 함수가 클램프한다(띠가 서로 겹치면 판별 자체가 불가능).
- **방향 반전**은 이 방식에선 보통 불필요하다 — "힌지의 반대편"이라는 규칙이 제품과 무관하게 물리적으로 고정이기 때문. (다른 세 방식은 "밝은 쪽이 힌지냐 립이냐"가 제품마다 달라 반전이 필요했다.)

**디버그 뷰로 튜닝하기** — 아래처럼 실행하면 "여는 방향" 을 누를 때마다 객체별로 계산 과정을 띄운다:

```bash
VISUPICK_OPENING_DEBUG=1 python main.py
```

`opening_analysis.debug_show_hinge` 가 두 패널을 보여준다:

| 패널 | 내용 |
|---|---|
| 왼쪽 | OBB 로 똑바로 세운 케이스(warp) 위의 **네 띠**. **빨강 = 힌지로 고른 변**, **초록 = 그 맞은편(여는 쪽)**, 회색 = 나머지. 어두운 부분은 마스크 밖(점수 계산에서 빠진 영역) |
| 오른쪽 | 네 변의 **점수 막대** + 채택된 단서(`cue = color` / `lightness`) / `conf` / `hinge=? -> open=?` |

글자는 **ASCII 만** 쓴다 — `cv2.putText` 는 한글을 못 그려 깨지기 때문. `T/R/B/L` = 위/오른쪽/아래/왼쪽(warp 기준)이다. 아무 키나 누르면 다음 객체로 넘어간다(다른 디버그 창과 동일).

**보는 법:** 힌지 변 하나만 막대가 길고 나머지 셋이 비슷하면 잘 동작하는 것이다(1등과 2등의 격차 = `conf`). 엉뚱한 변이 빨강이면 ① 왼쪽 패널에서 빨간 띠가 **실제 투명부를 덮고 있는지** 보고 `띠 두께%` 를 맞추고, ② 그래도 안 되면 마스크가 케이스를 정확히 감쌌는지(그림자를 먹지 않았는지) 확인한다.

> **이 방식은 정사각형 전용이 아니다** — 네 변을 모두 보므로 가로로 긴 케이스에도 쓸 수 있다. 다만 가로로 긴 케이스는 `grid` 가 실측에서 더 강건하므로 그쪽을 먼저 쓴다.
...
> **표시에는 캘리브레이션과 캡처가 모두 필요하다.** Bin Box 는 **base 좌표계**에 저장되므로, 화면에 그리려면 base→cam 변환(= 캘리브레이션)과 픽셀 투영(= 캡처로 얻는 intrinsics)이 있어야 한다. 값 자체는 시작할 때 `bin_box.json` 에서 이미 메모리로 올라와 있지만(`_load_bin_box`), 캘리브레이션을 로드하기 전에는 그릴 수가 없다 — 재시작 직후 "Bin Box 가 사라진 것처럼" 보이는 이유가 이것이다. `_refresh_bin_box_views()` 가 **표시 성공 여부를 반환**하고, 캡처 완료 메시지에 `⚠ 저장된 Bin Box 를 표시하지 못함: 캘리브레이션을 로드하세요` 를 붙여 이유를 알린다. (예전에는 `_render_bin_box` 가 띄운 경고가 바로 뒤의 "캡처 완료" 메시지에 덮여 원인을 알 수 없었다.)
