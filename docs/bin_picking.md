...
| **힌지 투명도** (`"hinge"`) ✅**정사각형 케이스 전용** | **4 (네 변)** | 힌지가 있는 변만 **투명**하다는 제품 특징을 이용. 네 변 안쪽 띠를 모두 평가해 가장 투명한 변 = 힌지, **그 반대쪽 = 여는 방향** | 힌지의 맞은편이 립 (반전 불필요) |
...
**"투명함"을 이미지에서 어떻게 재나 — 본체 색 대비:**

띠의 색이 **케이스 본체 색에서 얼마나 벗어났나**를 Lab 색상면(a\*b\*) **거리**로 잰다. 힌지 쪽은 투명하거나(바닥이 비침) 금속이라 **중성(회색)** 에 가깝고, 나머지 세 변은 본체 색(예: 분홍) 그대로다. 본체 표본은 띠를 뺀 **안쪽 영역의 중앙값**(인쇄·요철에 덜 흔들리게 median).

**왜 밝기(L)를 버리나:** 투명부는 배경 색을 보이되 플라스틱이 빛을 먹어 **어두워지므로**, 밝기까지 비교하면 본체와 구별되지 않는다. 게다가 케이스 자체의 요철 그늘·하이라이트가 밝기를 지배해 방향을 잃는다 — 실측에서 L 을 포함하자 왼쪽 음영에 속아 오답이 났다(`[12.9, 8.09, 8.44, 20.2]` 로 왼쪽이 1등). 색상면만 보면 같은 사진이 `[3.43, 0.89, 1.37, 1.91]` 로 깔끔히 갈린다.

**왜 배경을 안 쓰나:** 그림자는 배경과 색이 같아 "투명"으로 오인되기 쉽다. 본체를 기준으로 삼으면 배경 표본 자체가 필요 없다.

컬러가 없으면 밝기 편차로 대체하지만 판별력이 크게 떨어지므로 **컬러를 넘기는 것을 전제로 한다** (탭이 `rgb=self.current_rgb` 를 넘긴다).

> **제거된 대안 (실측 근거):** 예전에는 `배경 유사도`(띠가 바닥 색 분포와 닮은 정도)와 `이질도`(띠가 나머지 세 띠와 다른 정도)를 `판정` 콤보로 고를 수 있었다. 둘 다 히스토그램의 **피어슨 상관**으로 쟀는데, 이 지표는 두 분포가 겹치지 않으면 **"얼마나 먼지"와 무관하게 비슷한 값으로 포화**된다. 실제 캡처(분홍 정사각 케이스, 배경 RGB 130/129/128 · 힌지 103/94/96 · 본체 94/68/66)에서 네 변 점수가 `0.15~0.18` 안에 몰려 사실상 동점이 됐고 — **정지 장면인데도 실행마다 답이 바뀌고 정반대로도 나왔다.** 8회 반복 비교에서 `배경 유사도`는 실제 제품(0/8)과 검은 케이스(3/8)에서 실패했고, `이질도`는 합성에선 통했지만 실물에선 설정 따라 답이 튀었다(실제 장면은 프레임 요철·인쇄 때문에 "튀는 변"이 여럿이라 "제일 튀는 변 = 힌지" 가정이 깨진다). 본체 색 대비는 세 상황 모두 정답이었고, 색이 없는 케이스(흰·검정)에서는 **낮은 신뢰도로 정직하게 보고**한다 — 틀린 답을 확신하는 것보다 낫다. 선택지로 남기면 "그럴듯한데 틀리는" 길을 열어 두는 셈이라 제거했다. 무채색 케이스가 실제로 등장하면 **그 캡처를 받아 전용 단서를 설계**한다.

> **약점 — 마스크 품질이 전제다:** 배경 대신 본체를 기준으로 삼으므로, **마스크에 그림자가 섞이면** 그 변이 중성 회색이라 '힌지'로 오인된다(실측에서 확신하며 오답). 마스크 경계는 짧은 변의 **4%만큼 침식**해 걷어내지만 — 힌지 띠 자체가 가장자리에 붙어 있어 더 깎으면 신호까지 사라지므로 — 넓은 그림자까지는 못 막는다. SAM3 마스크가 케이스를 정확히 감싸는지 디버그 뷰에서 확인할 것.
>
> ⚠️ **침식 구현 주의:** `cv2.erode` 의 **기본 경계값은 최댓값**이라 이미지 가장자리가 깎이지 않는다. warp 마스크는 OBB 를 꽉 채우므로 기본값으로 부르면 **침식이 통째로 무효**가 된다(픽셀 수가 그대로). 실제로 이 버그 때문에 마스크가 2px 만 삐져나와도 그 변이 배경색을 머금어 점수가 `2.4 → 4.1` 로 뛰었고, 근거 없는 합성 장면에서 신뢰도가 0.45까지 나왔다. `borderType=cv2.BORDER_CONSTANT, borderValue=0` 을 반드시 넘긴다 (고친 뒤 같은 장면이 `0.001~0.017`).

**신뢰도(confidence)** 는 다른 방식과 의미가 다르다. 4지선다에서는 "얼마나 확실히 **하나만** 튀는가"가 곧 신뢰도이므로, 1등과 2등의 **절대 격차**를 Lab 색차 단위로 환산해 쓴다:

```
confidence = clip((1등 − 2등) / HINGE_CONF_LAB_SCALE, 0, 1)      # 상수 = 4.0
```

Lab 에서 1 단위는 겨우 구분되는 색차, 4~5 단위면 눈에 뚜렷한 차이 — 그 정도 격차면 확신해도 좋다는 뜻이다.
...
| 오른쪽 | 네 변의 **투명함 점수 막대** + `metric` / `conf` / `hinge=? -> open=?` |
