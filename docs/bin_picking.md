...
```
X: 412.0, Y: 233.0, Deg: 12.3, Open: 0.42
```

라벨은 픽셀 좌표(OBB 중심) · OBB 각도 · 여는 방향 신뢰도를 한 줄에 모은 것으로, 글자 길이에 맞춘 사각형을 **그 객체의 색으로 채우고 흰 글씨**로 그린다(`_draw_info_label`) — 객체가 여럿일 때 bbox·마스크와 같은 색이라 어느 라벨이 어느 객체 것인지 바로 매칭된다.
