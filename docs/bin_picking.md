...
**2D 뷰 표시 (`image_view.DraggableImageLabel`):** 회전 사각형(OBB) + **중심 → 여는 쪽 화살표** + 중심의 **정보 라벨**.

```
X: 412.0, Y: 233.0, Deg: 12.3, Open: 0.42
```

라벨은 픽셀 좌표(OBB 중심) · OBB 각도 · 여는 방향 신뢰도를 한 줄에 모은 것으로, 글자 길이에 맞춘 사각형을 **그 객체의 색으로 채우고 흰 글씨**로 그린다(`_draw_center_label`) — 객체가 여럿일 때 bbox·마스크와 같은 색이라 어느 라벨이 어느 객체 것인지 바로 매칭된다. 선택된 객체는 초록으로 강조된다. `Open:` 은 여는 방향을 계산했을 때만 붙는다(OBB 만 계산한 경우 생략). 이미지 가장자리 객체는 라벨이 잘리지 않도록 캔버스 안으로 밀어 넣는다.
