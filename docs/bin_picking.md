...
**2D 뷰 표시 (`image_view.DraggableImageLabel`):** 회전 사각형(OBB) + **중심 → 여는 쪽 화살표** + **정보 라벨**.
...
- **화살표를 먼저 그리고 라벨을 나중에** 그린다. OBB 루프에서는 `pending_labels` 에 모으기만 하고, 화살표 루프가 끝난 뒤 최상단에 올린다 — 반대 순서면 긴 화살표가 글씨 위를 지나 가린다. 선택된 객체는 초록으로 강조된다. `Open:` 은 여는 방향을 계산했을 때만 붙는다(OBB 만 계산한 경우 생략). 이미지 가장자리 객체는 라벨이 잘리지 않도록 캔버스 안으로 밀어 넣는다.
