...
### Z축 회전 제한 — 진공 호스 감김 방지 ⚠️

**문제:** 진공 호스가 로봇 팔을 따라 붙어 있어, 손목이 한 방향으로 계속 돌면 **줄이 감긴다**. 그런데 `compute_approach_pose` 의 **ABC unwrap**([calibration.py](../calibration.py))은 "현재 자세에 가장 가까운 표현"을 모듈로-360 으로 고르기 때문에, 예컨대 현재 A=170° 에서 목표가 −170° 면 **190°** 를 준다. 각도 자체는 같은 자세지만 손목은 실제로 계속 감기고, 반복되면 200°·300° 로 **누적**된다. (실제 테스트에서 +200° 이상으로 도는 상황이 포착됐다.)

**해결:** Grasp 설정에 **허용 A 범위**를 두고, 벗어나면 **같은 물리 자세의 다른 표현**으로 되돌린다 (`_limit_yaw`):

| 후보 | 의미 |
|---|---|
| `A ± 360k` | **완전히 동일한 자세** — 표현만 다르다 |
| `A ± 180` | 툴을 Z축으로 **뒤집은** 자세. 흡착 패드는 원형이라 **같은 지점을 잡는다** (선택적 허용) |

범위 안에 드는 후보 중 **현재 A 에서 가장 덜 도는 것**을 고르고, 하나도 못 맞추면 **그 파지를 배제**한다(자동 선택에서 자동으로 건너뛴다).

| 설정 | 기본 | 의미 |
|---|---|---|
| **Z 회전 제한 사용** | 켬 | 끄면 예전처럼 무제한 (감김 위험) |
| **허용 A 범위** | **−180 ~ +90°** | 이 범위를 벗어난 A 는 명령하지 않는다 |
| **180° 뒤집힌 자세도 허용** | 끔 | 켜면 배제가 줄지만 **툴 X 방향이 반대**가 되어 '여는 방향 정렬'이 뒤집힌다 |

> **범위는 장비마다 다르다 — 실측으로 정할 것.** 이 장비 기준으로 Home 의 TCP A 가 **−90°**, 물체는 **0~+90°** 에서 잡히는데 **A 가 +90° 를 넘는 순간 호스가 감긴다.** 그래서 상한을 **+90°** 로 뒀다. 다른 배선이면 이 값을 바꿔야 한다.
>
> 이 설정은 **`grasp_config.json` 에 저장**되어 재시작해도 유지된다 — 안전 설정이 기본값으로 되돌아가면 위험하기 때문이다.

> **트레이드오프:** 범위를 지키려면 때로 **먼 쪽으로 크게 돌아야** 한다(예: 170° → −170° 는 −340° 회전). 케이블 보호를 위한 의도된 동작이며, 회전량이 크면 선택 시 상태바에 "회전변화 N°" 경고가 함께 뜬다.

#### 이동 중 회전 방향은 아직 제어하지 못한다 (알려진 한계)

파지 **자세**를 범위 안에 넣는 것만으로는 호스 감김을 막지 못한다. 우리는 KRL 에 **`FRAME`(X,Y,Z,A,B,C)만 보내고 Status/Turn 은 KUKA 가 자동 선택**한다([kuka_robot.py](../kuka_robot.py) `_enqueue_motion`). 그런데 **`A=-170°` 와 `A=+190°` 는 같은 자세**라, 목표 A 를 어떻게 적든 **손목이 어느 쪽으로 돌지는 로봇의 관절 해 선택이 정한다.**

한때 회전을 여러 구간으로 쪼개 경유점을 넣어 방향을 강제했으나, **역효과가 있어 제거**했다: 로봇이 이미 음의 방향으로 크게 돌아 도달한 자세(예: Cartesian +120° = 실제 손목 −240°)에서 경유점이 다시 음의 방향으로 돌리려 해 **더 감기게** 만들었다. 우리가 **실제 손목 각을 모른 채** Cartesian 값만으로 방향을 지시했기 때문이다.

**실측으로 확인된 완화책 — 놓기 자세를 Home 과 같은 각으로 티칭:**

놓기 A 를 Home 의 A 와 같게 두면, 픽에서 어느 쪽으로 돌았든 **놓으러 갈 때 그 회전이 그대로 되풀려** 사이클마다 손목이 쉼 자세로 돌아온다(누적 0). 놓기가 Home 에서 멀수록 되감기지 못한 양이 쌓인다.

| 놓기 A (Home=−90° 기준) | 픽 45° 일 때 놓기 시점의 손목 이탈 |
|---|---|
| **−90° (Home 과 동일)** | **0°** ✅ |
| −160° | +290° ⛔ |
| ±180° | +270° ⛔ |

> **근본 해결(미구현):** `$AXIS_ACT` 로 **A6 관절각을 직접 읽고**([kuka_robot.py](../kuka_robot.py) `get_axis_position`), 목표 A6 를 Home 에 가장 가까운 표현으로 골라 **관절 제어**(`add_move_axis`)로 손목만 돌리면 모호성이 완전히 사라진다. 두 API 모두 이미 있으므로 필요해지면 그 방향으로 간다.
>
> **남은 엣지 케이스:** 픽 A 가 Home 에서 정확히 180° 떨어진 각도(이 장비에선 +90°)는 양쪽 회전량이 같아 **원리적으로 모호**하다.
...
### Home / Place(놓기) 위치 티칭과 영구 저장

**티칭(teaching)** 이란 로봇을 손으로(조그 버튼으로) 원하는 자리까지 움직여 놓고 "지금 이 자세를 기억해" 라고 저장하는 것을 말한다. 좌표를 숫자로 계산해 입력하는 대신 실제로 가 보고 기록하므로, 셀(작업 공간) 배치가 조금 달라도 바로 맞출 수 있다.

VisuPick 이 기억하는 티칭 위치는 두 개다 (둘 다 `main` 소유 — 탭 공용):

| 값 | 의미 | 재설정 버튼 |
|---|---|---|
| `main.home_pose` | 픽이 끝나고 돌아갈 대기 자세 | **📍 Home 재설정** |
| `main.place_pose` | 픽 사이클이 물체를 내려놓는 자리 | **📍 놓기 위치 재설정** |

두 값은 **`teach_poses.json` 에 저장되어 앱을 껐다 켜도 유지된다** (`grasp_config.json` · `bin_box.json` 과 같은 방식, gitignore 대상). 로봇 연결 시 동작은:

```
로봇 연결
 ├ teach_poses.json 에 값이 있으면 → 그 값을 그대로 사용 (현재 TCP 로 덮어쓰지 않음)
 └ 없으면(최초 실행)              → 연결 시점의 TCP 를 Home·놓기 위치로 잡고 파일에 기록
```

`_load_teach_poses` / `_save_teach_poses` ([main.py](../main.py)). 저장 파일이 손상됐을 때 로봇을 엉뚱한 좌표로 보내지 않도록 `_valid_pose` 가 `x y z a b c` 6개 키가 모두 숫자인지 검사하고, 아니면 무시하고 현재 TCP 로 폴백한다.

`get_tcp_position()` 은 KRL `$POS_ACT`(E6POS 타입)를 파싱하므로 위 6개 외에 **`s` `t`**(Status·Turn — 같은 좌표를 만드는 여러 관절 해 중 어느 것을 썼는지 나타내는 KUKA 고유 값)와 **`e1`~`e6`**(외부축)도 함께 들어온다. VisuPick 은 모션을 보낼 때 `FRAME`(x y z a b c)만 쓰므로 이 값들을 사용하지는 않지만, **저장·복원 때 버리지 않는다** — 새로 티칭한 포즈와 파일에서 복원한 포즈의 내용이 달라지면, 나중에 Status/Turn 을 쓰도록 바뀌었을 때 "직접 티칭하면 되는데 재시작하면 안 되는" 재현 불가 버그가 되기 때문이다.

**왜 바꿨나:** 예전에는 연결할 때마다 **그 순간의 TCP** 를 Home 으로 잡았다. 그래서 로봇이 어디에 서 있느냐에 따라 Home 이 매번 달라졌고, 놓기 위치는 재시작할 때마다 다시 티칭해야 했다. 셀 배치가 고정된 현장에서는 두 위치가 거의 바뀌지 않으므로 파일로 남기는 쪽이 맞다.

**주의:** 저장된 좌표는 **저장 당시의 Tool / Base 번호 기준**이다. Tool 이나 Base 를 바꿔 재연결하면 같은 숫자가 다른 자세를 뜻하게 되므로 두 위치를 다시 티칭해야 한다. 같은 이유로 연결을 해제해도 값을 지우지 않는다 (예전에는 해제 시 `home_pose = None` 이었다).

### 관절 좌표 티칭 — 진공 호스 꼬임 방지

Home / 놓기 위치는 Cartesian(`x y z a b c`)과 함께 **관절 각도(`home_axis` / `place_axis`, A1~A6)도 저장**되고, 두 위치로의 복귀는 관절값이 있으면 **관절 PTP**(`add_move_axis` → KRL `PTP robo_E6AXIS[...]`)로 실행된다.

**왜 관절 좌표인가 — 문제의 뿌리:** 지금까지의 모든 모션은 Cartesian `FRAME`(Status/Turn 없음)으로 보냈다. 같은 자세를 만드는 관절 해가 여러 개 있으므로 (`A=-170°` 와 `A=+190°` 는 같은 자세), 로봇이 "현재 관절에서 가까운" 해를 **알아서 선택**한다. A6(손목 롤)은 ±350° 까지 돌 수 있어서, 이 선택이 반복되면 진공 호스가 감기는 방향으로 계속 누적될 수 있었다.

관절 목표에는 이 선택이 존재하지 않는다: **관절값 `A6=+270°` 와 `A6=-90°` 는 플랜지 방향은 같아도 서로 다른 물리적 위치**(모터가 감긴 양이 다름)이므로, 관절 PTP 는 각 관절을 현재 각도에서 목표 각도로 **단조롭게** 보간할 뿐이다. 예: 현재 A6=+170° → 목표 −90° 이면 반드시 −260° 회전한다 ("+100° 짧은 길"로 가면 도착점이 +270° 가 되어 목표값이 아니므로 불가능). 그래서:

- **매 사이클 Home 복귀 = A6 완전 리셋** — 감김이 사이클 간 누적되지 않음
- 픽 도중 감긴 상태(예: A6=+300°)에서 Home(A6=−60°) 복귀는 −360° 회전 = **그 자체가 호스 풀기**
- 놓기 위치의 "A ±178~180 부호 요동" 문제도 소멸 (관절값엔 부호가 명확)
- 남는 것: 픽 타겟은 객체마다 달라 Cartesian 유지 — 단 Home 관절이 고정되므로 Home→픽 회전은 ±180° 이내로 유계

구현 위치: 저장/로드 [main.py](../main.py) (`AXIS_KEYS`/`_valid_axis` 검증 포함 — 손상 시 관절값만 무시하고 Cartesian 폴백), 단일 이동 [robot_control_mixin.py](../robot_control_mixin.py) `_move_to_taught` (확인 다이얼로그에 관절/Cartesian 어느 쪽인지 표시), 픽 사이클 `_build_pick_steps` 의 `("move_axis", axis_dict)` 스텝 + `_cycle_tick` 분기, 시퀀스 `_enqueue_home_to_sequence` (추가 시점 관절 스냅샷).

**구버전 파일 호환:** 관절값이 없는 `teach_poses.json` 은 그대로 동작하되 Cartesian PTP 로 폴백한다. 연결 시 관절값 부재를 감지하면 상태바에 재티칭 안내를 띄운다 — **로봇이 그 자리에 서 있다는 보장이 없으므로 현재 관절값으로 자동 보충하지 않는다** (엉뚱한 자세가 Home 이 되는 사고 방지). Home/놓기 **재설정 버튼을 한 번씩 다시 누르면** 관절값이 채워진다.

**검증 로그:** 관절 복귀를 큐에 넣기 직전 `A6 현재 → 목표 (Δ)` 를 로그로 남긴다. T1 저속으로 A≈+170° 객체를 픽한 뒤 Home 복귀에서 Δ가 감김을 푸는 방향으로 나오는지 숫자로 확인할 수 있다.

**검토했던 대안:** ① A6 소프트 리밋 축소(머신 데이터 `$SOFTN_END[6]`/`$SOFTP_END[6]`) — 산업 표준이고 가장 확실하지만 smartPAD Expert 권한으로 컨트롤러 설정을 바꿔야 함. ② `robo_E6POS` 를 FRAME→E6POS 로 바꿔 Status/Turn 전송 — KRL 계약 변경 + T비트 계산이 필요해 같은 효과를 더 비싸게 얻음. ③ 회전 피팅(rotary fitting) — 기계적 근본 해결이나 하드웨어 작업 필요. 소프트웨어만으로 되는 관절 티칭을 선택했고, 필요 시 ①을 병행할 수 있다.
...
비상정지(Space/버튼)를 누르면 진행 중이던 픽/시퀀스 사이클은 **폐기되지 않고 일시정지**된다 (`_cycle_pause`, [robot_control_mixin.py](../robot_control_mixin.py)):

1. **어디까지 했는지 역추적**: 전송해 둔 각 모션 슬롯의 `robo_motion_type` 을 읽어 "물리적으로 완료된 마지막 스텝"을 찾고, 첫 **미완료** 모션 스텝으로 `_cycle_idx` 를 되감는다 (이를 위해 pending 슬롯을 `(slot, step_idx)` 쌍으로 기록한다).
2. **KRL 큐 비우기**: 이미 배치해 둔 잔여 모션을 지운다 — 안 지우면 비상정지 해제 순간 로봇이 멋대로 이어서 움직인다.
3. **비상정지 해제 버튼**을 누르면 "이어서 진행하시겠습니까?" 다이얼로그가 뜬다. Yes → 0.5초 뒤(KRL 이 RESUME 으로 중단 모션을 정리할 여유) 미완료 스텝부터 재전송, **연속 픽이었다면 반복도 계속**된다. No → 사이클 완전 종료 (로봇은 현재 위치 유지).

되감은 모션의 재전송이 안전한 이유: 목표가 절대좌표라서, 중간에 멈춘 로봇이 같은 목표로 다시 가는 것뿐이다. 진공/dwell 스텝은 "모든 모션의 물리적 완료 후 실행" 규칙 때문에 되감기 구간에 끼어 있을 수 없고, dwell 도중 정지였다면 **남은 대기 시간**을 기억했다가 재개 때 다시 잰다.

> **수정 이력:** 예전에는 비상정지가 사이클을 완전 폐기(`_cycle_abort`)했다. 그런데 KRL 큐에 이미 배치된 모션은 해제 후 그대로 소진되므로, "정지 전 작업만 이어서 하고 그 다음은 사라지는" 반쪽 동작이 됐다 — 연속 픽 도중 비상정지→해제 시 루프가 재개되지 않는 문제. 사이클 **밖**(캡처/검출 단계)에서의 비상정지는 로봇이 정지 상태이고 보존할 모션이 없으므로 지금도 루프를 종료한다 (▶ 버튼으로 재시작).

---

## 7.6 연속 픽 (자동 반복) — 폐루프 빈 비우기

지금까지의 픽은 **사용자가 객체를 고르고 버튼을 누르는** 단발 동작이었다. 시퀀스 큐로 여러 개를 예약할 수는 있지만, 그건 **추가 시점의 좌표**를 그대로 쓰기 때문에 하나를 집어 더미가 무너지면 나머지 좌표가 어긋난다 (§10 주의).

**연속 픽**은 이 문제를 없앤 폐루프다 — **매 사이클마다 다시 보고 다시 고른다**:

```
① 캡처 → ② SAM3 검출 → ③ 여는 방향 → ④ 자동 선택
   → ⑤ Approach → 하강 → 진공 ON → 상승 → 놓기 → 진공 OFF + 블로우 → Home
   → ①로 돌아가 반복
```

우측 패널 **"연속 픽 (자동 반복)"** 그룹의 `▶ 연속 픽 시작`.

### 구현 — 기존 사이클 위에 얹은 바깥 루프

픽 자체는 검증된 `_build_pick_steps` + `_run_cycle` 상태머신을 **그대로 재사용**하고, 그 위에 반복을 얹었다. 이를 위해 `_run_cycle` 에 **완료/중단 콜백**을 추가했다([robot_control_mixin.py](../robot_control_mixin.py)):

- `on_done` → 사이클이 끝나면 `_auto_after_pick` 이 호출돼 **다음 사이클을 예약**(`QTimer.singleShot`).
- `on_abort` → 비상정지·오류로 사이클이 깨지면 **루프도 즉시 멈춘다**. (이게 없으면 사이클만 죽고 루프가 계속 돌아 위험)

캡처·SAM3 추론은 블로킹이라 각 단계 사이에 `QApplication.processEvents()` 로 UI 를 살려두고, 단계마다 `_auto_running` 을 재확인해 **정지 버튼이 즉시 먹도록** 한다.

### 자동 선택 기준 (`_auto_pick_index`)

후보마다 **실제 파지 TCP**를 `_compute_grasp_tcp`(§6.7 Grasp 설정 반영)로 구한 뒤 정렬한다 — 로봇이 실제로 갈 지점 기준이라 표시·이동과 일관된다. **Z 안전 한계**(파지점·Approach 둘 다)를 못 넘는 후보는 애초에 제외한다.

| 기준 | 정렬 | 언제 |
|---|---|---|
| **가장 위 (Z 최대)** ⭐기본 | base Z 내림차순 | 빈피킹 표준 — 아래 것부터 건드리면 더미가 무너진다 |
| 검출 신뢰도 최고 | `confidence` | 인식이 확실한 것부터 |
| 여는 방향 신뢰도 최고 | `opening.confidence` | 방향 판정이 뚜렷한 것부터 (§3.4) |
| 빈 중심에 가까움 | Bin Box 중심(§6.8)과의 거리 | 벽에서 먼 것부터 |
...
|---|---|
| **검출 0개** | 빈 비움 완료 (정상 종료) |
| **최대 반복 도달** | 폭주 방지 상한 (기본 10) |
| 파지 가능 후보 없음 | 전부 Z 한계 미달 / 자세 계산 실패 |
| `⏹ 정지` 버튼 | 진행 중 단계를 마치는 대로 중단 |
| **Space 비상정지** | 즉시 정지. 사이클 도중이면 **일시정지** — 해제 시 재개 여부를 물어 Yes 면 루프까지 계속, No 면 종료. 캡처/검출 단계면 루프 종료 (§7.5 픽 사이클의 "비상정지 = 일시정지" 참고) |

> **Space 단축키 주의(수정 이력):** Space 는 Qt 에서 **포커스된 버튼을 누르는 기본 키**다. 그래서 예전처럼 탭 안에만 단축키를 걸어두면, 버튼에 포커스가 있을 때 비상정지 대신 **그 버튼이 눌린다** — 실제로 '로봇 연결' 버튼이 눌려 **연결이 끊기는** 사고가 있었다. 지금은 (a) 단축키를 **메인 윈도우에 하나만** 등록해 현재 탭으로 디스패치하고, (b) **모든 버튼을 `NoFocus`** 로 만들어 어디에 포커스가 있든 Space 가 항상 비상정지로 동작한다. 텍스트 입력칸에서는 공백이 정상 입력된다.

### 시작 전 검증 (자율 동작이므로)

로봇·카메라·캘리브레이션·**놓기 위치**·SAM3 프롬프트를 모두 확인하고, 속도·반복 수·전체 순서를 보여주는 **확인 다이얼로그**를 거친다. Home 이 저장돼 있지 않으면 "복귀 없음" 을 경고에 명시한다.

> **아직 없는 것 (한계):** 흡착 성공 여부를 확인하지 못한다 — ZSE 압력 스위치가 `$IN` 에 매핑돼 있지 않아 소프트웨어가 읽을 수 없다(하드웨어 배선/매핑 필요). 그래서 **파지 실패를 감지하거나 재시도하지 못하고**, 빈 손으로 놓기 동작을 수행할 수 있다. 이 센서가 연결되면 실패 감지 → 다음 후보 재시도 → 성공률 자동 집계까지 이어질 수 있다.
...
### 동작만이 아니라 **레이아웃도** Mixin 이 소유한다

처음 추출했을 때는 *메서드(동작)* 만 옮기고, `로봇 이동 제어` / `시퀀스 큐` 그룹박스를 **만드는 코드**는 각 탭의 `_init_ui` 에 복붙된 채로 남겨 뒀다. 그 결과:

- 빈 픽킹 탭에서 버튼 위치를 바꿔도 CAD 매칭 탭은 **그대로**여서 매번 손으로 두 번 고쳐야 했고,
- 실제로 배치가 어긋났다 — 같은 버튼인데 이름이 `btn_move_robot`(빈 픽킹) / `btn_move`(CAD) 로 달랐고, 접근/철수 툴팁 문구도 서로 달랐다.

"같은 UI 인데 두 벌 존재"하는 상태 자체가 드리프트의 원인이므로, 레이아웃 생성도 Mixin 으로 올렸다:

| 빌더 | 만드는 것 | 쓰는 탭 |
|---|---|---|
| `_build_move_group()` | `로봇 이동 제어` 그룹박스 **통째로** | 빈 픽킹, CAD 매칭 |
| `_build_seq_group()` | `시퀀스 큐` 그룹박스 **통째로** | 빈 픽킹, CAD 매칭 |
| `_build_home_row()` | `[🏠 Home으로 이동 \| 📍 Home 재설정]` 한 줄 (2:1) | 세 탭 |
| `_build_place_row()` | `[📦 놓기 위치로 이동 \| 📍 놓기 위치 재설정]` 한 줄 (2:1) | 빈 픽킹, CAD 매칭 |
| `_build_vacuum_row()` | 진공 ON/OFF/블로우 + 픽 실행 (2행) | 세 탭 |
| `_build_safety_rows(layout)` | 큐 비우기 / 비상정지 / 비상정지 해제 | 세 탭 |

덕분에 각 탭의 `_init_ui` 는 두 줄로 끝난다:
...
`로봇 이동 제어` 그룹의 세로 순서 (두 탭 동일):

```
방식(LIN/PTP) → 속도(%) → 접근/철수 → Z 최소(mm) → 흡착대기(s)
→ 선택 위치로 이동
→ [Home으로 이동 | Home 재설정]
→ [놓기 위치로 이동 | 놓기 위치 재설정]
→ 진공 ON/OFF/블로우  +  픽 실행(잡기→놓기)
→ 큐 비우기 / 비상정지 / 비상정지 해제
```

**왜 표면 추적 탭은 통째로 안 쓰나:** 이 탭의 그룹은 이름부터 `실행 제어` 고, "선택 위치로 이동" 대신 `시작 / 정지` 버튼을 쓰며, 이동 방식은 LIN 고정이고 시퀀스 큐가 없다. 억지로 한 빌더에 넣으면 "이 탭에서는 이걸 숨김" 플래그만 늘어나므로, **하위 빌더(Home 행 · 진공 행 · 안전 행)만 골라 쓰고** 나머지는 자기 그룹에 직접 만든다. 대신 믹스인이 참조하는 위젯 중 이 탭에 없는 것(`use_approach`, `approach_dist`, `action_list` 등)은 숨김 더미로 채워 둔다 ([surface_tracking_tab.py](../surface_tracking_tab.py)).

**중복 방지 플래그:** 진공 행은 원래 `픽 실행 | 흡착대기 | 놓기 위치 저장` 이 한 줄에 모여 있었다. `_build_move_group()` 은 흡착대기를 `Z 최소` 밑으로, 놓기 버튼을 `Home` 행 밑으로 옮겨 따로 만들기 때문에, 같은 위젯이 두 번 생기지 않도록 `PLACE_BUTTONS_IN_OWN_ROW` / `DWELL_SPIN_IN_OWN_ROW` 를 `True` 로 세팅한 뒤 `_build_vacuum_row()` 를 호출한다. 이 플래그가 `False` 인 표면 추적 탭은 예전처럼 한 줄에 모인 형태를 유지한다. 두 위젯이 빠진 빈 픽킹 / CAD 매칭에서는 `픽 실행` 버튼이 `stretch=1` 로 행 전체를 채운다.
