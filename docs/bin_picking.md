...
**신뢰도(confidence)** 는 다른 방식과 의미가 다르다. 4지선다에서는 "얼마나 확실히 **하나만** 튀는가"가 곧 신뢰도이므로, 1등과 2등의 **격차를 전체 산포로 정규화**한 값(0~1)을 쓴다:

```
confidence = (1등 − 2등) / (1등 − 4등)
```

네 변이 고만고만하면 0에 가깝고, 하나만 뚜렷하게 투명하면 1에 가깝다. **0.15 미만이면** 판별이 애매하다는 뜻 → `띠 두께%` 를 실제 투명부 폭에 맞추거나 `판정` 기준을 바꿔본다. (합성 검증에서는 힌지 변 0.99 / 나머지 −0.09 로 갈려 신뢰도 ≈ 1.0.) 상태바에 이 값의 범위를 `힌지 판별 격차 0.32~0.71` 형태로 함께 표시한다.
