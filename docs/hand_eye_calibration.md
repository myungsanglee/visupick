...
이 행렬 하나로 회전과 이동을 동시에 표현한다.

**좌표계(frame)란?** 원점 + XYZ 축 방향의 묶음이다. 로봇 베이스에 하나, 그리퍼에 하나, 카메라에 하나, 체커보드에 하나 — **같은 물리적 점이라도 어느 좌표계에서 재느냐에 따라 숫자가 달라진다**. 변환 행렬은 그 숫자를 다른 좌표계의 숫자로 바꿔주는 "번역기"다.

**표기 읽는 법 (이 코드베이스 전체 공통):** `T_a2b` = "a 좌표계의 점을 b 좌표계 숫자로 바꾸는 변환".

```python
p_in_b = T_a2b @ p_in_a          # 점 옮기기 (p 는 (x,y,z,1) 동차좌표)
T_a2c  = T_b2c @ T_a2b           # 연쇄: a→b→c. 행렬은 오른쪽부터 적용된다
T_b2a  = np.linalg.inv(T_a2b)    # 역행렬 = 반대 방향 변환
```

`2`(to) 를 기준으로 안쪽 글자가 맞물리면 (`b2c @ a2b` → `a2c`) 옳게 합성한 것이다. 이 문서(그리고 다른 문서들)의 모든 수식은 이 규칙 하나로 검산할 수 있다.
...
**이 식이 왜 성립하는지 (Eye-in-Hand 로 유도):** 체커보드는 월드에 고정이므로, 어느 자세에서 계산하든 "베이스 기준 체커보드 자세"는 같아야 한다:

```
T_target2base = T_gripper2base(i) · X · T_target2cam(i)     ← 모든 자세 i 에서 동일한 값
```

(오른쪽부터 읽으면: 체커보드→카메라→그리퍼→베이스. 1절의 합성 규칙 그대로다.) 자세 i 와 j 에 대해 등식을 놓고 정리하면:
...
즉 **A = "로봇이 얼마나 움직였나"** (로봇이 보고한 TCP 로 계산), **B = "카메라가 본 세상이 얼마나 움직였나"** (체커보드 측정으로 계산). 미지수는 X 하나뿐이고, 자세 쌍이 많을수록 과결정계가 되어 노이즈에 강해진다 — 3.2절에서 자세를 10개 이상 다양하게 모으라는 이유다.
...
저장 디렉토리 구조 ("새 세션 시작" 시 폴더명을 물어본다 — 기본값은 `session_YYYYMMDD_HHMMSS`, 원하는 이름으로 변경 가능):
```
...
수집 품질 관리용 UI 기능 (수집된 포즈 테이블):

- **더블클릭** → 그 포즈에 저장했던 이미지와 체커보드 검출 결과를 다시 보여줌 (리뷰 모드 — 카메라 연결 없이도 동작). "캡처"를 누르면 실시간 화면으로 복귀.
- **Delete 키 또는 행의 ✕ 버튼** → 잘못 수집한 포즈를 폴더째 삭제. 삭제된 번호는 재사용하지 않아 남은 데이터와 섞이지 않는다 (번호에 빈 구멍이 생겨도 캘리브레이션은 폴더 스캔 방식이라 무해).
...
Eye-to-Hand 모드에서는 전자를 역행렬로 뒤집어서 `T_base2gripper` 형태로 넣는다.

**왜 뒤집으면 같은 함수로 풀리나?** 두 모드는 "무엇이 고정이고 무엇이 움직이는가"만 대칭으로 바뀐 문제다:

| | 체커보드 | 카메라 |
|---|---|---|
| Eye-in-Hand | 월드에 고정 | 그리퍼에 부착 (움직임) |
| Eye-to-Hand | 그리퍼에 부착 (움직임) | 월드에 고정 |

그리퍼→베이스 변환을 역행렬로 뒤집어 "베이스가 그리퍼 기준으로 움직인 것"처럼 보면, Eye-to-Hand 문제가 Eye-in-Hand 와 **정확히 같은 AX=XB 꼴**이 된다. OpenCV 의 `calibrateHandEye` 가 모드 인자 없이 함수 하나만 제공하는 이유이고, 입력만 바꿔 넣으면 되는 이유다.
...
### 4.7 결과 리포트 — 무엇을 어떻게 풀었는지 보여주기

`compute_hand_eye(return_metric=True)` 는 변환 행렬만이 아니라 **과정을 설명하는 리포트**를 함께 반환하고, 데이터 수집 탭의 결과 창이 이를 표시한다:

| 항목 | 키 | 의미 |
|---|---|---|
| 실제 자세추정 방식 | `pose_method_counts` | `auto` 로 돌렸을 때 **실제로** 3D 직접 매칭을 썼는지 solvePnP 로 폴백했는지 (포즈 수까지) |
| 채택된 알고리즘 | `algorithm` / `algorithm_initial` | OpenCV 5종 중 최종 채택(예: `DANIILIDIS+NLO`)과 1차 선택 |
| ① 1차 오차 | `metric_initial_raw` | OpenCV 알고리즘 결과 그대로 (전체 포즈, 정밀화 전) |
| ② 정밀화 후 | `metric_initial` | 비선형 정밀화(4.5) 적용 후 (전체 포즈) |
| ③ 최종 오차 | `metric_mean` | greedy outlier 제거(4.6)까지 마친 값 |
| 제외된 포즈 | `n_removed`, `removed_poses` | 몇 개를 왜 뺐는지 추적 |

실측 예 (33 포즈, eye-to-hand, Zivid):

```
포즈 추정: auto → 3D 직접 매칭(SVD) 33개
사용 포즈: 16 / 33   (outlier 제외 17개: pose_017, pose_005, ... 외 12개)
Hand-eye 알고리즘: DANIILIDIS+NLO
  ① 1차 (OpenCV DANIILIDIS 결과) : 0.488 mm
  ② 비선형 정밀화(NLO) 후        : 0.316 mm
  ③ 최종 (outlier 제거 후)       : 0.147 mm
  → 개선: 0.488 → 0.147 mm (70% 감소)
```

**왜 이걸 보여주나:** `auto` 는 편하지만 "그래서 무엇을 썼는지" 감춰버린다. 또 최종 숫자만 보면 **개선 알고리즘(4.5·4.6)이 실제로 기여했는지** 알 수 없다. 세 단계 오차를 나란히 보여주면 어느 단계가 효과가 있었는지, 포즈를 너무 많이 버리지는 않았는지 사용자가 판단할 수 있다.
