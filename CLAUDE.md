...
- [tracking.py](tracking.py): **객체 추적** — 검출 결과에 `track_id` 를 붙여 프레임 사이로 이어 붙인다. `ObjectTracker` 인터페이스 아래 `ByteTrackTracker`(2단계 연관)와 `BotSortTracker`(+ GIoU, CMC 옵션, **ReID 제외**), `create_tracker(name)` 팩토리. **검출기 비종속** — 입출력이 `object_detector` 의 검출 표준 포맷이라 SAM3·RF-DETR·앞으로 추가될 검출기 모두에서 동작한다. 새 의존성 없음(scipy 헝가리안 할당). ⚠️ **roboflow/trackers 는 설치 금지** — numpy>=2 요구가 SAM3(numpy<2)와 충돌하고 opencv-contrib 를 깨뜨린다 (requirements.txt 참고).
